...
from diffusers import DDIMScheduler, StableDiffusionXLPipeline, UNet2DConditionModel
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = self.scheduler.step(self.scheduler, noise_pred, t, latents, **extra_step_kwargs)[0]
...
class DDPOStableDiffusionXLPipeline:
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents_dtype = latents.dtype
                scheduler_output = scheduler_step(noise_pred, t, latents, **extra_step_kwargs)[0]
        ...
        return DDPOPipelineOutput(images=image, latents=all_latents, log_probs=all_log_probs)
...
class DefaultDDPOStableDiffusionXLPipeline(DDPOStableDiffusionXLPipeline):
    ...
    def load_checkpoint(self, models, input_dir):
        ...
        if self.use_lora and hasattr(models[0], "peft_config") and getattr(models[0], "peft_config", None) is not None:
            state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(models[0]))
            self.sd_pipeline.load_lora_weights(input_dir, weight_name="pytorch_lora_weights.safetensors")
        elif not self.use_lora and isinstance(models[0], UNet2DConditionModel):
            models[0].load_pretrained(os.path.join(input_dir, "unet"))
        else:
            raise ValueError(f"Unknown model type {type(models[0])}")
